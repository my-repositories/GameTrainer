...
namespace gt::lua {
LuaWrapper::LuaWrapper() : LuaWrapper(luaL_newstate()) {}
...
LuaWrapper::~LuaWrapper() { lua_close(this->state); }

void LuaWrapper::registerFunction(const char *name,
                                  void (*callback)(xml::CheatEntry *,
                                                   float)) const {
    ...
    lua_pushcclosure(
        ...
        [](lua_State *luaState) -> int {
            ...
            if (lua_gettop(luaState) != 2 || !lua_isuserdata(luaState, -2) ||
                !lua_islightuserdata(luaState, callBackIndex)) {
                return 0;
            }

            auto func = (void (*)(xml::CheatEntry *, float))lua_touserdata(
                luaState, callBackIndex);
        ...
        [](lua_State *luaState) -> int {
            if (lua_gettop(luaState) == 1 && lua_isstring(luaState, -1)) {
                ...
                if (lua_islightuserdata(luaState, userDataIndex)) {
                    auto func = (void (*)(const char *))lua_touserdata(
                        luaState, userDataIndex);
...
void LuaWrapper::registerFunction(const char *name,
    ...
    lua_pushcclosure(
        ...
        [](lua_State *luaState) -> int {
            ...
            if (lua_islightuserdata(luaState, userDataIndex)) {
                auto func = (int (*)(lua_State *, const char *))lua_touserdata(
                    luaState, userDataIndex);
                const char *data = lua_tostring(luaState, -1);
                func(luaState, data);
            }
