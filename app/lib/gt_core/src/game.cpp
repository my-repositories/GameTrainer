...
void Game::updateValue(const gt::xml::CheatEntry *entry,
                       const float valueToAdd) {
    //        if (temp) // "entry" == "Float"
    //        {
    const DWORD_PTR ptr = this->getValueAddress(entry);
    float currentValue = 0;

    this->osApi->readProcessMemory(this->process, (LPVOID)ptr, &currentValue,
                                  entry->size, nullptr);
    currentValue += valueToAdd;
    this->osApi->writeProcessMemory(this->process, (LPVOID)ptr, &currentValue,
                                   entry->size, nullptr);
    //        }
}
...
DWORD_PTR Game::getValueAddress(const gt::xml::CheatEntry *entry) const {
    ...
    this->osApi->readProcessMemory(this->process, (LPCVOID)baseAddress,
                                  &valueAddress, dwSize, nullptr);
    ...
    for (int i = 1; i < entry->offsetsCount; i++) {
        DWORD_PTR offset =
            valueAddress + entry->offsets[entry->offsetsCount - i];
        this->osApi->readProcessMemory(this->process, (LPCVOID)offset,
                                      &valueAddress, dwSize, nullptr);
    }
...
BOOL Game::isWow64() const {
    ...
    fnIsWow64Process = (LPFN_ISWOW64PROCESS)this->osApi->getProcAddress(
        this->osApi->getModuleHandle(TEXT("kernel32")), "IsWow64Process");
